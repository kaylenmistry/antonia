defmodule AntoniaWeb.CoreComponents do
  @moduledoc """
  ...
  attr :timeout, :integer,
    default: nil,
    doc: "the time in milliseconds before the flash message automatically dismisses"
        ...
        >
          <div class="flex items-start">
            <div class="flex-shrink-0">
              ...
              />
              <.icon :if={@kind == :error} name="hero-x-circle-mini" class="h-6 w-6 text-red-600" />
            </div>
            ...
            <div class="ml-4 flex flex-shrink-0">
              <button
                type="button"
                class="group focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 inline-flex rounded-md"
                aria-label={gettext("close")}
              >
  ...
  def flash_group(assigns) do
    ~H"""
    <.flash kind={:info} title="Success!" flash={@flash} timeout={3_000} />
    <.flash kind={:error} title="Error!" flash={@flash} timeout={7_000} />
    <.flash
      id="client-error"
      kind={:error}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(AntoniaWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(AntoniaWeb.Gettext, "errors", msg, opts)
    end
  end
